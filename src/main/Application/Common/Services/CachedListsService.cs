...
public class CachedListsService : ICachedListsService
{
	...
	public async Task<List<TEnt>> GetList<TEnt>(CancellationToken token = default) 
		where TEnt : class
	{
		var ret = await _cache.GetOrAddAsync(CacheKey<TEnt>(), () =>
			GetListInner<TEnt>(token), DefaultAbsoluteExpiration);
		return ret;
	}

	private async Task<List<TEnt>> GetListInner<TEnt>(CancellationToken token = default)
		where TEnt : class
	{
	...
	public async Task<IReadOnlyDictionary<int, TEnt>> GetMap<TEnt>(CancellationToken token = default)
		where TEnt : class, IIdentifiable
	{
		var list = await GetList<TEnt>(token);
		var map = list.ToDictionary(e => e.Id, e => e);
		return map;
	}

	public async Task<IEnumerable<IStandardListItem>> GetStandardListItems<TEnt>(CancellationToken token = default) where TEnt : 
